...
export const LeadTable: React.FC<LeadTableProps> = ({ onSelectLeadDetail }) => {
  ...
  const renderChannelBadge = (source: string) => {
    if (source.includes('Instagram') || source.includes('Meta')) {
      return (
        <span className="inline-flex items-center space-x-1 px-2.5 py-0.5 rounded text-xs font-bold bg-pink-100 text-pink-900 border border-pink-300 whitespace-nowrap">
    ...
    if (source.includes('Google')) {
      return (
        <span className="inline-flex items-center space-x-1 px-2.5 py-0.5 rounded text-xs font-bold bg-sky-100 text-sky-900 border border-sky-300 whitespace-nowrap">
    ...
    if (source.includes('Homepage') || source.includes('Website')) {
      return (
        <span className="inline-flex items-center space-x-1 px-2.5 py-0.5 rounded text-xs font-bold bg-indigo-100 text-indigo-900 border border-indigo-300 whitespace-nowrap">
    ...
    if (source.includes('Brochure')) {
      return (
        <span className="inline-flex items-center space-x-1 px-2.5 py-0.5 rounded text-xs font-bold bg-amber-100 text-amber-900 border border-amber-300 whitespace-nowrap">
    ...
    return (
      <span className="inline-flex items-center space-x-1 px-2.5 py-0.5 rounded text-xs font-bold bg-slate-100 text-slate-900 border border-slate-300 whitespace-nowrap">
    ...
    <div className="ls-card overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-xs text-slate-900 whitespace-nowrap">
          ...
          {/* Table Header */}
          <thead className="bg-slate-100 text-[11px] font-bold text-slate-600 uppercase tracking-wider border-b border-slate-300">
            <tr>
              ...
              <th scope="col" className="py-3 px-4 min-w-[260px]">Student / Contact</th>
              <th scope="col" className="py-3 px-4 min-w-[300px]">Course Program</th>
              ...
              <th scope="col" className="py-3 px-4 min-w-[160px]">Assigned Counsellor</th>
              <th scope="col" className="py-3 px-4 min-w-[130px]">Created Date</th>
              <th scope="col" className="py-3 px-4 text-right min-w-[120px]">Actions</th>
            </tr>
          </thead>

          {/* Table Body (Normal Comfortable Text) */}
                  ...
                  >
                    ...
                    <td className="py-3 px-4">
                      <select
                        ...
                        className={`text-[11px] font-bold px-2 py-0.5 rounded border focus:outline-none transition-all cursor-pointer whitespace-nowrap ${getStatusBadgeStyle(lead.status)}`}
                      >
                    ...
                    {/* Date Added */}
                    <td className="py-3 px-4 text-[11px] text-slate-600 font-mono font-bold whitespace-nowrap">
      ...
      {/* Pagination Footer */}
      <div className="bg-slate-100 px-5 py-2 border-t border-slate-300 flex items-center justify-between text-xs text-slate-700 font-bold">
        <div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center space-x-1.5">
            <button
              ...
              className="px-2 py-0.5 bg-white text-slate-800 rounded border border-slate-300 disabled:opacity-40 hover:bg-slate-50 font-bold text-xs"
            >
              Prev
            </button>

...
export const LeadTable: React.FC<LeadTableProps> = ({ onSelectLeadDetail }) => {
  ...
  return (
    <div className="clean-card overflow-hidden">
            ...
            <tr>
              <th scope="col" className="p-3.5 w-10 text-center">
                <input
                  ...
                  className="rounded border-slate-700 bg-slate-950 text-indigo-600 focus:ring-indigo-500 h-4 w-4 cursor-pointer"
                />
              </th>
              ...
              <tr>
                <td colSpan={8} className="py-12 text-center text-slate-500">
                  <div className="flex flex-col items-center justify-center space-y-1.5">
                    ...
                    <p className="font-medium text-slate-400 text-xs">No leads found matching current filters.</p>
                  </div>
                </td>
              </tr>
            ) : (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center space-x-2.5">
                        <div className="h-8 w-8 rounded-full bg-slate-800 flex items-center justify-center text-slate-200 font-bold text-xs shrink-0 border border-slate-700">
                          {lead.name.charAt(0)}
                        </div>
                        <div>
                          <div className="flex items-center space-x-1.5">
                            <span 
                              ...
                              className="font-bold text-white hover:text-indigo-400 cursor-pointer transition-colors text-xs"
                            >
                              {lead.name}
                            </span>
                            ...
                            {lead.isDuplicate && (
                              <span 
                                onClick={() => onSelectLeadDetail(lead)}
                                className="px-1.5 py-0.5 rounded text-[9px] font-extrabold bg-rose-500/20 text-rose-300 border border-rose-500/30 cursor-pointer"
                              >
                                DUP
                              </span>
                            )}
                    ...
                    {/* Course */}
                    <td className="py-3 px-4">
                      <div className="font-semibold text-slate-200 text-xs truncate max-w-[180px]" title={lead.course}>
                        {lead.course}
                      </div>
                    ...
                    <td className="py-3 px-4">
                      <select
                        ...
                        className={`text-[11px] font-semibold px-2 py-1 rounded-lg border focus:outline-none transition-all cursor-pointer ${getStatusBadgeStyle(lead.status)}`}
                      >
                        ...
                        {assignedCounsellor ? (
                          <>
                            ...
                            <select
                              ...
                              className="bg-transparent text-xs text-slate-300 font-medium focus:outline-none hover:text-white cursor-pointer"
                            >
                              {counsellors.map((c) => (
      ...
      {/* Pagination Footer */}
      <div className="bg-slate-900/60 px-4 py-2.5 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
        <div>
          Showing <span className="font-bold text-slate-200">{leads.length > 0 ? startIndex + 1 : 0}</span> to{' '}
          <span className="font-bold text-slate-200">{Math.min(startIndex + itemsPerPage, leads.length)}</span> of{' '}
          <span className="font-bold text-slate-200">{leads.length}</span> entries
        </div>
